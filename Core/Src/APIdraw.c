...
{
    int color = kleur_omzetter(kleur_str);
    ErrorList errors = Error_handling(FUNC_tekst, x, y, color,(int)tekst_str, (int)fontnaam, schaal_factor, (int)fontstijl, 0, 0, 0, 0);
    if (errors.error_var1 != NO_ERROR || errors.error_var2 != NO_ERROR ||errors.error_var3 != NO_ERROR || errors.error_var4 != NO_ERROR || errors.error_var5 != NO_ERROR ||errors.error_var6 != NO_ERROR || errors.error_var7 != NO_ERROR)
    {
        return errors;
    }
    ...
    while (tekst_str[current_char_index] != '\0') {
        ...
        if (is_cursief) char_width += (int)((7.0 / 3.0) * schaal_factor + 0.5); // correcte afronding
        char_width += schaal_factor;

        // --- Wrap naar volgende regel automatisch ---
        ...
        for (int rij = 0; rij < BASE_FONT_SIZE; rij++) {
            ...
            int offset_schuin = 0;
            if (is_cursief) offset_schuin = (int)((rij / 3.0) * schaal_factor + 0.5);
